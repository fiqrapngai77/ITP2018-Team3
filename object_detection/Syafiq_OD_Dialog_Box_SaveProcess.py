...
###Intiializes the selected directory
if len(tempdir) > 0:
    IMAGE_DIR = tempdir

TO_SAVE_DIR = tempdir + "\\PROCESSED\\"
...
###Iterates through the entire folder for every image
for FILENAME in os.listdir(IMAGE_DIR):
    if FILENAME.endswith(".jpg") or FILENAME.endswith(".jpeg"):
        ...
        ###The directory to save the file + the filename
        FINAL_PATH = TO_SAVE_DIR + FILENAME
        ###Save the file
        cv2.imwrite(FINAL_PATH ,image)

        print ("Scanning " + FILENAME + "...")
        totalCountofObjects = [category_index.get(value) for index,value in enumerate(classes[0]) if scores[0,index] > 0.8 ]
        print(str(len(totalCountofObjects)) + " insects detected")
        print ("------------")
        ...
        print (FILENAME + " is not of the proper filetype")
